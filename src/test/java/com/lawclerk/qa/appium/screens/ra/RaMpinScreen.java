...
/**
 * Screen object for the RA app's MPIN entry step, shown after a successful
 * email/password login. Same shared component as HA's MPIN screen -
 * identical accessibilityIdentifiers, confirmed via a live UI tree dump
 * (LAW-881). The submit button sits under the on-screen keypad, so the
 * keyboard's "Done" accessory must be dismissed first or the tap misses it.
 */
public class RaMpinScreen extends BaseScreen {
    ...
    public RaMpinScreen(IOSDriver driver) {
        super(driver);
    }

    public RaMpinScreen enterMpin(String mpin) {
