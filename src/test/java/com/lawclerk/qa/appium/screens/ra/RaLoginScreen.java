...
/**
 * Screen object for the RA (Remote Associate) app's login screen. The email
 ...
 * dump - see LAW-881). The submit button is labeled "Sign In" here, unlike
 * HA's "LOGIN" - same shared component library, different button copy.
 */
public class RaLoginScreen extends BaseScreen {
    ...
    private static final By SUBMIT_BUTTON = AppiumBy.accessibilityId("Sign In");
    ...
    public RaMpinScreen submit() {
        dismissKeyboard();
        click(SUBMIT_BUTTON);
        return new RaMpinScreen(driver);
    }
